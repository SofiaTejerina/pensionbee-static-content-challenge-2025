...
jest.mock("../../src/app/mdx-components", () => ({
...
jest.mock("../../src/app/[...content]/mdxLoader", () => ({
    loadMDX: (path: string) =>
        path === "/about-page"
            ? Promise.resolve({
                __esModule: true,
...
describe("Dynamic MDX Page", () => {
    it("renders “Acme Co.” when the MDX exists", async () => {
        ...
        render(element);
        expect(await screen.findByTestId("content")).toHaveTextContent("Acme Co.");
    });
    ...
    it("renders a 404 when the MDX is missing", async () => {
        ...
        const element = await Page({ params });
        render(element);
        expect(await screen.findByTestId("not-found")).toHaveTextContent("404 Not Found");
    });
});